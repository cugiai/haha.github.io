...
function contentinfoWindow(s, nameHpt, addressHpt, imageHpt, nameDistrict, phoneHpt, webHpt){
   var html = '<div>'+
    '<div>'+
        '<a href="'+webHpt+'"><strong style="color:blue;font-size:17px">'+nameHpt+'</strong></a>'+
    '</div>'+
    '<div>'+
        '<img src="'+imageHpt+'" style="width:400px;height:150px; margin-left:30px">'+
    '</div>'+
    '<div>'+
        '<strong style="background-color:blue;margin-left:60px;">Bác Sĩ:</strong>'+
    '</div>'+
    '<div>'+
        '<strong style="margin-left:40px;">'+addressHpt+', '+nameDistrict+', Hà Nội, Việt Nam'+'</strong><br/>'+
        '<strong style="margin-left:40px;color:blue;"><u>0'+phoneHpt+'</u></strong><br/>'+
        '<a href="http://'+webHpt+'/"  target="blank" style="margin-left:40px;color:blue;">'+webHpt+'</a><br/>'+
        '<strong style="margin-left:40px;">Giờ mở cửa: 8h-17h</strong><br/>'+
    '</div>'+
    '<div style="width:400px; height:80px; margin-left:30px;background-color:#ccc;border: 1px solid transparent;border-radius: 2px 0 0 2px;box-sizing: border-box;-moz-box-sizing: border-box;outline: none;box-shadow: 0 2px 6px rgba(0, 0, 0, 0.3);">'+
        '<strong style="margin-left:40px;">Các khoa của bệnh viện</strong><br/>'+
        s+
    '</div>'+
'</div>';
return html;
            ...
            if(placeIdDB===place.place_id){
              ...
              for(var j = 0; j<khoa.length; j++){
                ...
                if(idHpt_khoa===idHpt){
                    ...
                    s+='<span style="margin-left:20px;width:150px;color:blue;float:left"><a class="listKhoa" value="'+idHpt+'" href="DoiNguBacSiChuyenKhoa.php?idHpt='+idHpt+'">'+nameKhoa[k-1]+'</a></span>';
                }
              }
            }
          }
    ...
    Array.from(classname).forEach(function(element) {
      element.addEventListener('click', function(){
        ...
        thongtinchitiet.innerHTML='<div style="font-size:12px;padding-top:15px">'+html+
                                  '<img id="thoatDS" src="icon/thoat.png" style="width: 20px;height: 20px; position:absolute;right:0px;top:0px">'+
                                  '</div>';
